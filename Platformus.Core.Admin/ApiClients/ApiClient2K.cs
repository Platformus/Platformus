...
public class ApiClient2K<TKey1, TKey2, TDto, TFilter> : IApiClient2K<TKey1, TKey2, TDto, TFilter>
  ...
  where TFilter: class, IFilter
{
  ...
  public ApiClient2K(HttpClient httpClient) : this(httpClient, typeof(TDto).GetCustomAttribute<MagicalizedAttribute>()?.Route ?? string.Empty) { }

  public ApiClient2K(HttpClient httpClient, string urlSegment)
  {
    this.httpClient = httpClient;
    this.urlSegment = urlSegment;
  }

  public virtual async Task<TDto?> GetByIdAsync(TKey1 id1, TKey2 id2, params Inclusion<TDto>[] inclusions)
  {
    string uri = $"{urlSegment}/{id1}/{id2}?fields={string.Join(',', inclusions.Select(i => i.PropertyPath).ToList())}";
  ...
  {
    HttpResponseMessage response = await this.httpClient.DeleteAsync($"{urlSegment}/{id1}/{id2}");
