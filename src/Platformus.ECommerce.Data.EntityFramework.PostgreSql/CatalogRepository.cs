...
namespace Platformus.ECommerce.Data.EntityFramework.PostgreSql
{
  ...
  /// for manipulating the <see cref="Catalog"/> entities in the context of PostgreSQL database.
  /// </summary>
  public class CatalogRepository : RepositoryBase<Catalog>, ICatalogRepository
  {
    ...
    public void Delete(Catalog catalog)
    {
      this.storageContext.Database.ExecuteSqlCommand(
        @"
          CREATE TEMP TABLE ""TempCatalogs"" (""Id"" INT PRIMARY KEY);
          WITH RECURSIVE ""X"" AS (
            SELECT ""Id"" FROM ""Catalogs"" WHERE ""CatalogId"" = {0}
            UNION ALL
            SELECT ""Catalogs"".""Id"" FROM ""Catalogs"" INNER JOIN ""X"" ON ""Catalogs"".""CatalogId"" = ""X"".""Id""
          )
          INSERT INTO ""TempCatalogs"" SELECT ""Id"" FROM ""X"";
          CREATE TEMP TABLE ""TempDictionaries"" (""Id"" INT PRIMARY KEY);
          INSERT INTO ""TempDictionaries"" VALUES ({1});
          INSERT INTO ""TempDictionaries"" SELECT ""NameId"" FROM ""Catalogs"" WHERE ""Id"" IN (SELECT ""Id"" FROM ""TempCatalogs"");
          DELETE FROM ""Catalogs"" WHERE ""Id"" IN (SELECT ""Id"" FROM ""TempCatalogs"");
