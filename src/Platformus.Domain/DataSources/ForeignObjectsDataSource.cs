...
namespace Platformus.Domain.DataSources
{
  public class ForeignObjectsDataSource : DataSourceBase
  {
    public override IEnumerable<SerializedObject> GetSerializedObjects(IRequestHandler requestHandler, SerializedObject serializedPage, params KeyValuePair<string, string>[] args)
    {
      if (!this.HasArgument(args, "OrderBy") || !this.HasArgument(args, "Direction"))
      {
        if (this.HasArgument(args, "MemberId"))
          return requestHandler.Storage.GetRepository<ISerializedObjectRepository>().Foreign(CultureManager.GetCurrentCulture(requestHandler.Storage).Id, this.GetIntArgument(args, "MemberId"), serializedPage.ObjectId).ToList();
      ...
      if (this.HasArgument(args, "MemberId"))
        return requestHandler.Storage.GetRepository<ISerializedObjectRepository>().Foreign(CultureManager.GetCurrentCulture(requestHandler.Storage).Id, this.GetIntArgument(args, "MemberId"), serializedPage.ObjectId, dataType.StorageDataType, orderBy, direction).ToList();
    ...
    public override IEnumerable<Object> GetObjects(IRequestHandler requestHandler, Object page, params KeyValuePair<string, string>[] args)
    {
      ...
      if (this.HasArgument(args, "MemberId"))
        return requestHandler.Storage.GetRepository<IObjectRepository>().Foreign(this.GetIntArgument(args, "MemberId"), page.Id, dataType.StorageDataType, orderBy, direction, CultureManager.GetCurrentCulture(requestHandler.Storage).Id).ToList();
