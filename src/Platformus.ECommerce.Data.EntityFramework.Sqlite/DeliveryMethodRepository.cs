...
namespace Platformus.ECommerce.Data.EntityFramework.Sqlite
{
  ...
  /// for manipulating the <see cref="DeliveryMethod"/> entities in the context of SQLite database.
  /// </summary>
  public class DeliveryMethodRepository : RepositoryBase<DeliveryMethod>, IDeliveryMethodRepository
  {
    ...
    {
      return this.dbSet.FirstOrDefault(dm => dm.Code.ToLower() == code.ToLower());
    }
    ...
    public void Delete(DeliveryMethod deliveryMethod)
    {
      this.storageContext.Database.ExecuteSqlRaw(
        @"
          ...
          INSERT INTO TempDictionaries SELECT NameId FROM DeliveryMethods WHERE Id = {0};
    ...
    private IQueryable<DeliveryMethod> GetFilteredDeliveryMethods(IQueryable<DeliveryMethod> deliveryMethods, string filter)
    {
      ...
      return deliveryMethods.Where(dm => dm.Code.Contains(filter.ToLower()));
    }
  }
}