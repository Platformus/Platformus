...
namespace Platformus.ECommerce.Data.EntityFramework.SqlServer
{
  ...
  /// for manipulating the <see cref="PaymentMethod"/> entities in the context of SQL Server database.
  /// </summary>
  public class PaymentMethodRepository : RepositoryBase<PaymentMethod>, IPaymentMethodRepository
  {
    ...
    public void Delete(PaymentMethod paymentMethod)
    {
      this.storageContext.Database.ExecuteSqlCommand(
        @"
          ...
          INSERT INTO #Dictionaries SELECT NameId FROM PaymentMethods WHERE Id = {0};
    ...
    private IQueryable<PaymentMethod> GetFilteredPaymentMethods(IQueryable<PaymentMethod> paymentMethods, string filter)
    {
      ...
      return paymentMethods.Where(pm => pm.Code.ToLower().Contains(filter.ToLower()));
    }
  }
}